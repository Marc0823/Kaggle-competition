...
import subprocess  # for kaggle CLI if needed
...
MODEL_DIR_ROOT = Path('/kaggle/input/notebooks/hongweiluan/rogii-wellbore-v6-gpu-script')
MODEL_DIR      = MODEL_DIR_ROOT / 'models'

# ── Unzip models if needed ────────────────────────────────────────────────────
import zipfile as _zf
MODEL_DIR.mkdir(exist_ok=True)
_zip = MODEL_DIR_ROOT / 'models.zip'
if _zip.exists() and not (MODEL_DIR / 'blend_weights.json').exists():
    print('Extracting models.zip...', flush=True)
    with _zf.ZipFile(str(_zip)) as _z: _z.extractall(str(MODEL_DIR))
...
CNN_WEIGHT = blend_info.get('cnn_weight', 0.0)  # 0 if CNN gate failed in training
N_FOLDS    = 5
...
oof_meta = pd.read_parquet(MODEL_DIR_ROOT / 'oof_meta.parquet')
...
_tcn_zip_candidates = [
    Path('/kaggle/input/rogii-tcn-v3-models/tcn_config.json'),
    Path('/kaggle/input/rogii-tcn-v3-models/rogii-tcn-v3-models/tcn_config.json'),
    Path('/kaggle/input/datasets/hongweiluan/rogii-tcn-v3-models/tcn_config.json'),
]
_found_zip = next((p for p in _tcn_zip_candidates if p.exists()), None)
print(f'TCN zip search: {[(str(p), p.exists()) for p in _tcn_zip_candidates]}', flush=True)
TCN_DIR = _found_zip.parent if _found_zip else Path('/kaggle/input/rogii-tcn-v3-models')
tcn_pred = np.zeros(len(test_df), np.float32)
tcn_available = False
try:
    import torch, torch.nn as nn, torch.nn.functional as F, zipfile as _zf2, json as _tcn_json
    _tcn_tmp = TCN_DIR
    # Use the zip file we already located (or fail loudly if not found)
    _tcn_zip = TCN_DIR
    # if _tcn_zip is None:
    #     raise FileNotFoundError('tcn_models.zip not found — add rogii-tcn-v3-models dataset to kernel inputs')
    # # print(f'TCN zip: {_tcn_zip} ({_tcn_zip.stat().st_size//1e6:.0f}MB)', flush=True)
    # if not (_tcn_tmp / 'tcn_config.json').exists():
    #     with _zf2.ZipFile(str(_tcn_zip)) as _z: _z.extractall(str(_tcn_tmp))
    if (_tcn_tmp / 'tcn_config.json').exists():
        ...
        class _TB(nn.Module):
            def __init__(self,c,k,d):
                super().__init__()
                self.cf=_SC(c,k,d); self.cg=_SC(c,k,d)
                self.res=nn.Conv1d(c,c,1); self.skip=nn.Conv1d(c,c,1)
            def forward(self,x):
                h=torch.tanh(self.cf(x))*torch.sigmoid(self.cg(x))
        ...
        for _f in range(_nf):
            _mp=_tcn_tmp/f'tcn_fold{_f}_best.pt'
            if not _mp.exists(): _mp=_tcn_tmp/f'tcn_fold{_f}.pt'
            if _mp.exists():
                ckpt = torch.load(str(_mp),map_location=_dev)
                sd = ckpt["state_dict"] if "state_dict" in ckpt else ckpt

                new_sd = {}
                for k, v in sd.items():
                    k2 = k.replace(".conv_f.", ".cf.")
                    k2 = k2.replace(".conv_g.", ".cg.")
                    new_sd[k2] = v
                _m=_TM(); _m.load_state_dict(new_sd, strict=False); _m.eval(); _folds.append(_m)
        ...
        for wid in test_df['well'].unique():
            try:
                ...
                sig=raw[['GR','ANCC','Z']].interpolate(limit_direction='both').fillna(0.).values.astype(np.float32)
                for _c in range(3):
                ...
                X=np.stack([sig[:,0],sig[:,1],sig[:,2],tvt_f,tvt_m,np.sin(md*2*np.pi),np.cos(md*2*np.pi)],axis=0)
                X_t=torch.from_numpy(X).unsqueeze(0)
                wgrp=test_df[test_df['well']==wid]
                # RevIN denorm using anchor rows
                anc_delta=kn['TVT_input'].values-last_tvt_w
                w_mean=float(np.mean(anc_delta)); w_std=float(np.std(anc_delta))+1e-6
                fp=[]
                with torch.no_grad():
                    for _m in _folds:
...
if tcn_available and (tcn_pred!=0).mean()>0.5:
    ...
    print(f'50% TCN + 50% GBM blend applied', flush=True)
else:
